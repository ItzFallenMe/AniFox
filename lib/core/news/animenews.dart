...
class AnimeNews {
  ...
  Future<Map<String, String?>> getDetailedNews(String url) async {
    ...
    final pageTitle = document
        ...
        .trim();
    final postedOn =
        document.querySelector('div#page-title > small')?.text.trim();
    final postedBy = document
        .querySelector('div#page-title')
        ?.text
        .replaceAll(RegExp(r'n| {2,}'), '')
        .split('by')[1]
        .trim();
    ...
    final image =
        details?.querySelector('figure > img')?.attributes['data-src'] != null
            ? _cdnUrl +
                (details
  ...
  Future<List<Map<String, String?>>> getNewses() async {
    ...
    document.querySelectorAll('.herald.box.news.t-news').forEach((element) {
      ...
      final ref = titleElement?.attributes['href'] != null
          ? url + (titleElement?.attributes['href'] ?? '')
          : null;
      final title = titleElement?.text.trim();
      final dateAndTime =
          wrapDiv?.querySelector('time')?.attributes['datetime'];
      final dateSplit = dateAndTime?.split('T')[0].split('-');
      final date =
          "${dateSplit?[2] ?? null}-${dateSplit?[1] ?? null}-${dateSplit?[0] ?? null}";
  ...
  Future<String> fetch(String url) async {
    final res =
        await get(Uri.parse(url), cacheDuration: const Duration(minutes: 5));
    return res.body;
  }
}
